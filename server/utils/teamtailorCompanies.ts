...
/**
 * Load TeamTailor companies from JSON file
 * This file is auto-generated by scripts/discover-all-teamtailor.js
 */
function loadCompaniesFromJSON(): TeamTailorCompany[] {
  try {
    const jsonPath = path.join(process.cwd(), 'teamtailor-companies.json')
    const data = JSON.parse(fs.readFileSync(jsonPath, 'utf8'))
    
    // Filter: only companies with at least 1 job and enabled = true
    const companies = data.companies
      .filter((c: any) => c.enabled && c.jobCount > 0)
      .map((c: any) => ({
        name: c.name,
    ...
    console.log(`✅ Loaded ${companies.length} active TeamTailor companies from JSON`)
    return companies
  } catch (error) {
    console.warn('⚠️ Could not load teamtailor-companies.json, using fallback list')
    return FALLBACK_COMPANIES
  }
}

/**
 * Fallback list of Swedish tech companies using TeamTailor
 * Used if JSON file cannot be loaded
 */
const FALLBACK_COMPANIES: TeamTailorCompany[] = [
...
// Load companies from JSON (or fallback)
export const TEAMTAILOR_COMPANIES: TeamTailorCompany[] = loadCompaniesFromJSON()
